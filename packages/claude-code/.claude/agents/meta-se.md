...
| `hld-design` | 无 BLOCKING 缺失信息 | 调用 `hld-designer`，输出 `.output/HLD.md`，其中必须包含候选架构方案对比、推荐方案和架构评审所需章节 | 写完 `HLD.md` 后立即停止，等待 meta-po 发起 HLD 确认 |
...
| Skill | 何时调用 | 产出 | 不适用边界 |
|-------|---------|------|-----------|
| `hld-designer` | 进入 solution-design，需要输出正式 HLD 时 | `.output/HLD.md` | REQUIREMENTS / USE-CASES 未确认时不要调用 |
