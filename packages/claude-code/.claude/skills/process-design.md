...
| 过滤规则名称 | 空 | 参数 | 无效 |
| 规则条件格式 | src_ip=10.0.0.1（合法格式） | 参数 | 有效 |
...
```
最终用例集 = C-01（P1正常创建）+ C-02（P2覆盖保存）+ C-03（P3取消）+ C-04（P4格式错误）
覆盖策略说明：每条路径各取一个代表值，P1覆盖正常，P2/P3覆盖条件分支，P4覆盖异常
```
...
|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|------------|------------|--------|---------|----------|
| 日志中心 | 日志管理 | 日志过滤配置 | 正常创建日志过滤规则 | PC-FLT-RUL-001 | P1 | 单台防火墙 | | 防火墙已正常启动；无同名过滤规则 | 1.进入日志过滤规则配置页面<br>2.点击"新建规则"<br>3.输入规则名=rule_new, 条件=src_ip=10.0.0.1<br>4.点击"确定"<br>5.查看规则列表 | 1.显示规则列表<br>2.显示规则配置表单<br>3.表单接受输入<br>4.提示"规则创建成功"<br>5.rule_new 显示在列表中 | V60R001C01 | | 日志过滤,规则创建 | 功能 | 否 |
