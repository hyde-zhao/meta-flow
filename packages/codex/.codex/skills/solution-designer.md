...
读取已确认的 `REQUIREMENTS.md`，先提炼问题边界，再生成至少 2 个候选方案与推荐结论，并将结果写入 `.output/HLD.md`。该 Skill 是旧触发词到 HLD 流程的兼容入口；若需要正式 HLD 交付，优先使用 `hld-designer`。
...
- [ ] `.output/REQUIREMENTS.md` 存在且 `confirmed=true`
...
6. 将结果写入 `.output/HLD.md`，设置 `status: ready-for-review` 与 `confirmed: false`
