...
| 状态 | 说明 | 允许操作 |
|------|------|---------|
| `draft` | Story 初始草稿，由 meta-se 生成 | 审查、修改 |
| `approved` | meta-po 确认 Story 边界，可进入 LLD 设计 | meta-dev 起草 LLD |
| `ready-for-lld-review` | meta-dev 已输出 LLD，等待人工确认 | meta-po 发起 LLD 审核 |
| `lld-approved` | LLD 已获人工确认，可开始开发 | meta-dev 将其推进到 `in-development` |
...
缺少任一件套，Story 不得从 `approved` 状态推进到 `ready-for-lld-review`。
...
- `approved` 之后必须先经过 `ready-for-lld-review` 和 `lld-approved`，才能进入 `in-development`
