...
每个检查点触发时，**必须使用 `ask_user` 工具，并提供结构化选项**，禁止用开放式提问代替。每个检查点对应的标准问法如下：
...
问法：
> "请确认以下需求分析结果是否满足预期，选择操作："

选项：
1. ✅ 确认通过 — 需求完整无歧义，进入方案设计阶段
2. ❌ 确认不通过 — 需重新分析，返回 requirement-clarification
...
问法：
> "以下是 [N] 个备选方案，请选择您认可的方案，或提出调整建议："

选项：
1. 选择方案 A — [方案A 名称简述]
2. 选择方案 B — [方案B 名称简述]
3. （如有更多方案，依次列出）
4. ✏️ 需要调整 — 请输入对现有方案的修改意见，meta-se 调整后再次确认
5. 🔄 重新设计 — 现有方案均不满足需求，返回重新设计（会触发失败模式检测）
...
问法：
> "以下是 Story 执行计划，请确认 Wave 分组和边界是否合理："

选项：
1. ✅ 确认通过 — Story 计划合理，开始 Wave 1 开发
2. ❌ 确认不通过 — 需重新规划，返回 story-planning
3. ✏️ 需要调整 — 请输入需要调整的 Story 边界或优先级，meta-se 调整后再次确认
...
选项：
1. ✅ 确认通过 — 交付内容完整，标记项目为 delivered
2. ❌ 确认不通过 — 请指出不完整之处，返回对应阶段补充
3. ✏️ 需要补充 — 请输入需要补充的文档或产物，meta-doc 补充后再次确认

---

**用户选择"需要补充"后的处理规则：**
- 接收用户输入的补充内容
- 追加到对应产物文件（不重写全部，只追加增量）
- 唤醒对应功能 Agent 处理
- 处理完成后**再次触发同一检查点**，展示更新后内容让用户重新确认
