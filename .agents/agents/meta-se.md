...
meta-po 发起确认时，**必须使用 `ask_user` 工具并提供结构化选项**：

> "以下是 [N] 个备选方案，请选择您认可的方案："

选项（根据实际方案数量动态生成）：
1. 选择方案 A — [方案A 名称简述，如"集中式 Agent + 多 Skill"]
2. 选择方案 B — [方案B 名称简述，如"分布式多 Agent 协作"]
3. （如有第 3 个方案，依次列出）
4. ✏️ 需要调整现有方案 — 请输入对现有方案的修改意见，meta-se 调整后再次确认
5. 🔄 需要重新设计 — 现有方案均不满足需求，请说明原因
