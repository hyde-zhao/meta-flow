...
**状态路径枚举**（每条路径 = 一个状态迁移场景）：

| 路径编号 | 路径描述 | 转换序列 | 路径类型 |
|---------|---------|---------|---------|
| SP1 | 完整生命周期 | T1→T2→T4→T6→T7 | 主路径（全周期） |
| SP2 | 创建后修改再启用 | T1→T5→T2 | 分支路径 |
| SP3 | 非法启用（未配置） | T8 | 负面路径 |
| SP4 | 非法禁用（未配置） | T9 | 负面路径 |
...
|---------|--------|--------|------------|---------|---------|
| C-01 | SP1 | 10.0.0.1 | 创建→启用→禁用→重启→删除 | 各步骤状态正确迁移 | T1,T2,T4,T6,T7 |
| C-02 | SP2 | 10.0.0.1 | 创建→修改→启用 | 状态：已配置→已配置→已启用 | T1,T5,T2 |
...
|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|------------|------------|--------|---------|----------|
| 日志中心 | 日志服务器 | 服务器管理 | 日志服务器完整生命周期 | PC-SRV-STA-001 | P1 | 单台防火墙直连日志服务器 | | 防火墙已正常启动；无已配置的日志服务器 | 1.创建日志服务器 IP=10.0.0.1:514<br>2.启用日志服务器<br>3.禁用日志服务器<br>4.重新启用日志服务器<br>5.删除日志服务器 | 1.状态变为"已配置"<br>2.状态变为"已启用"<br>3.状态变为"已禁用"<br>4.状态变为"已启用"<br>5.状态变为"未配置" | V60R001C01 | | 状态机,生命周期 | 功能 | 否 |
