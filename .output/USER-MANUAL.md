...
**输入**：特性需求文件  
**输出**：`mfq/feature-input/raw-requirements.md` + `directory-structure.md`
**确认点**：目录结构
...
**输入**：需求 + 目录 + 场景  
**输出**：`mfq/m-analysis/test-points.md`
...
```bash
# 读取耦合矩阵
python scripts/excel_coupling_tool.py read "耦合矩阵.xlsx" --output "mfq/f-analysis/coupling-graph.json"

# 查询某特性的耦合点
python scripts/excel_coupling_tool.py query "mfq/f-analysis/coupling-graph.json" --feature "日志"
