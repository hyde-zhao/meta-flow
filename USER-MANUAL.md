...
1. 确保仓库根目录包含：
   ...
   - `.agents/agents/mfq-test-designer.md`
...
- `.agents/agents/mfq-test-designer.md` → `.claude/agents/mfq-test-designer.md`
...
根据确认的设计方法，对每个逻辑用例执行四步设计（详见[第 4 章](#4-设计方法详解)）。
...
| 文档 | 内容 |
|------|------|
| `<特性名>特性测试方案.md` | 概述 + 场景 + 需求 + MFQ 分析 + 整合 + 覆盖率 |
...
1. **等价类划分表**
   ...
   |---------|-----------|-----------|---------|---------|
   | IP 地址 | [1]合法IPv4 [2]合法IPv6 | [3]空值 [4]非法格式 | 边界值 | 见结果 |

2. **数据组合分析表**

   | 数据项 | 取值 | 组合分析 | 约束条件 | 组合策略 | 组合结果 |
   |--------|------|---------|---------|---------|---------|
   | IP+端口+协议 | 见上 | 3×4×2=24 | 无 | pairwise | 12组 |

3. **逻辑用例**：每组数据对应一条逻辑用例
4. **物理用例**：含优先级 + 测试类型 + 完整步骤

**组合爆炸处理**：全组合 > 50 时自动建议 pairwise 压缩。
...
可以。design-planner 会先推荐方法，但用户可以在确认时修改任何逻辑用例的设计方法。
...
| 文件 | 说明 |
|------|------|
| `.agents/agents/mfq-test-designer.md` | Agent 核心提示词 |
| `.agents/skills/<name>/SKILL.md` × 14 | 14 个 Skill 定义 |
