...
def cr_trait_from_mapping(mapping: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(nested, dict):
        return {
            ...
            if _normalize_mapping_value(value) != ""
        }
    trait: dict[str, Any] = {}
...
def normalize_profile_stage(stage: str | dict[str, Any], human_gates: set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": mode,
        "human_gate": _default_human_gate(checkpoint, mode, required),
    }
...
def optional_gate_auto_clean_decision(
    *,
    ...
    if precheck_decision not in {"PASS", "WAIVED"}:
        blockers.append(
            f"{checkpoint} precheck_decision is {precheck_decision}, expected PASS or WAIVED"
        )
    if decision_count != 0:
...
def _stage(
    checkpoint: str,
    profile_stages: dict[str, dict[str, Any]],
    *,
    mode: str | None = None,
    ...
    base = dict(
        profile_stages.get(checkpoint)
        or {"checkpoint": checkpoint, "mode": "standard", "human_gate": "none"}
    )
...
def _applies(stage: dict[str, Any], reason: str = "") -> dict[str, Any]:
    payload = {
        "applies": True,
        "mode": stage["mode"],
        "human_gate": stage.get("human_gate", "none"),
    }
    if reason:
...
def _has_product_baseline_impact(
    cr_type: str, impact_surface: list[str], product_baseline_refresh_required: bool
) -> bool:
...
def _cp2_human_gate(
    ...
    if authz_policy_refs:
        return "required"
    if _as_bool(trait.get("uses_existing_evidence_only")) and not _as_list(
        trait.get("existing_evidence_refs")
    ):
        return "required"
    return "optional"
...
def derive_route_plan(
    *,
    ...
    applicability: dict[str, dict[str, Any]] = {
        checkpoint: _n_a("not included in gate profile") for checkpoint in CHECKPOINTS
    }
    ...
    cp1_required = gate_profile in {
        "architecture-major",
        "runtime-high-risk",
    } or _has_product_baseline_impact(cr_type, impact, product_baseline_refresh_required)
    if cp1_required:
        apply_stage(
            _stage("CP1", profile_stages), "required by gate profile or product baseline refresh"
        )
        if "CP1" not in profile_stages:
            warnings.append(
                "CP1 added because product baseline refresh or product-scope requires it"
            )
    else:
    ...
    has_new_verification = (
        None if has_new_verification_raw is None else _as_bool(has_new_verification_raw)
    )
    ...
    if uses_existing and any((has_new_design, has_new_impl, has_new_verification is True)):
        blockers.append(
            "uses_existing_evidence_only conflicts with new design/implementation/verification traits"
        )
    ...
    if uses_existing:
        ...
        if has_new_design or requires_arch:
            apply_stage(
                _stage(
                    "CP3",
                    profile_stages,
                    mode="standard" if requires_arch else None,
                    human_gate="required" if requires_arch else None,
                ),
                "new design or architecture review required",
                upgrade_reason="architecture review required"
                if requires_arch
                else "new design requires design review",
                recommended_profile=PROFILE_UPGRADE_TARGETS["architecture_review"]
                if requires_arch
                else PROFILE_UPGRADE_TARGETS["design"],
            )
        if requires_story_decomp:
            ...
            if cp4_stage.get("_outside_profile"):
                warnings.append(
                    "CP4 added outside gate_profile because story decomposition was requested"
                )
            apply_stage(
        ...
        if has_new_impl:
            if "CP5" in profile_stages:
                apply_stage(
                    profile_stages["CP5"], "implementation design evidence required by gate profile"
                )
            ...
            if has_new_verification is False:
                if waiver_reason and waiver_ref:
                    applicability["CP7"] = _waived(
                        "new implementation verification explicitly waived", waiver_ref
                    )
                else:
                    blockers.append(
...
def validate_route_plan_for_cr(
    ...
    *,
    ...
    if actual.get("warnings"):
        warnings.append(
            f"{cr_id} route_plan_ref has warnings: {', '.join(str(item) for item in actual.get('warnings') or [])}"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command not in {"plan", "check"}:
        raise SystemExit(
            ...
            "c0-dry-run, c0-apply, c0-cutover-plan, c0-cutover-apply"
        )
    parser = argparse.ArgumentParser(prog=f"meta-flow route {command}")
    parser.add_argument("--project-root", type=Path, default=Path.cwd())
    ...
    if command == "check":
        ...
        errors, warnings = validate_route_plan_for_cr(
            parsed.project_root, parsed.from_cr, profiles_data=profiles_data
        )
