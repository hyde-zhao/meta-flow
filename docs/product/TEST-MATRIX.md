...
## CR-076 增量追踪（CP2 scope baseline revision 4 落轴；canonical 条目见 docs/product/SCENARIOS.yaml v1.6 SCN-076-01..11 与 docs/product/REQUIREMENTS.md「CR-076 增量需求」节；确认随 CP2-CR-076 approve 生效。S05-A/B/C 为 STORY-CR076-S05 内部 milestone，不是独立 Story identity）

| 场景 | REQ | Story | 类型 | 当前状态 | 验证入口 / 边界 |
|---|---|---|---|---|---|
| SCN-076-01 | REQ-076-S01 | STORY-CR076-S01 | 风险分类 | planned | G0/G1/G2/unknown risk reason 正负向；ordinary 不自动升级 G2；public/security/production fail closed；G0/G1 N/A CP8 ≠ N/A authorization |
| SCN-076-02 | REQ-076-S02 | STORY-CR076-S02 | 授权输入 | planned | --authorization-file/ref/id：exactly-one/none/多选/错误 namespace/过期/复用/operation mismatch/OID drift 全负向 |
| SCN-076-03 | REQ-076-S03 | STORY-CR076-S03 | 资产身份 | planned | wheel/sdist 缺失/重复/错误版本/错误项目名/损坏/checksum/sidecar mismatch；source→accepted→published→installed 四重 identity 可追溯 |
| SCN-076-04 | REQ-076-S04 | STORY-CR076-S04 | 安装/升级 | planned | clean-home/已有安装/升级/降级/回滚/重复安装幂等/部分失败恢复；rollback 基线版本与状态/schema 回退边界 |
| SCN-076-05 | REQ-076-S04 | STORY-CR076-S04 | 权限/边界 | planned | user/project scope、权限不足、symlink/outside path、缓存污染 |
| SCN-076-06 | REQ-076-S05 | STORY-CR076-S05（milestone A） | 验收合同 | planned | ConsumerAcceptanceResultV1 schema/digest/OID/environment/command identity 校验；自然语言回传不采信；导入前 fail-closed |
| SCN-076-07 | REQ-076-S05 | STORY-CR076-S05（milestone B-1） | source-candidate replay 硬门 | blocked-until-authorized | 输入：冻结 source OID 的 source candidate（W3-W10 旅程，六轮×J1/J2/J3 明细，CR-073 先例 fixture-constructed journey 可作行证据）。identity：source OID+tree digest+consumer project UID+quant-lab 双仓 OID+command identity+profile/environment fingerprint。失效条件：source OID 漂移、任一旅程失效、环境/命令漂移 → 旧 result 失效重跑；不阻断 provider 制品构建准备，但阻断 S05 milestone B 验收。无独立 typed authorization（authorization_inherited=false）即 BLOCKED/not-executed；结论仅 PASS/BLOCKED。时序：16 步顺序步骤 10（source-sync push 后、artifact 传输前） |
| SCN-076-08 | REQ-076-S05 | STORY-CR076-S05（milestone B-2） | installed-artifact replay 硬门 | blocked-until-authorized | 输入：exact content-addressed bundle（wheel/sdist+receipt/sidecar 各自 SHA-256，单次构建）。identity：release/process source OID、SemVer、wheel/sdist/receipt/sidecar digest、consumer project UID、quant-lab 双仓 OID、command identity、environment fingerprint。失效条件：consumer 本地重建、任何 digest 变化、OID/环境/命令漂移 → 旧 result 失效进入新 lineage（重走 freeze→build→transport→replay）；源码 PASS、fixture PASS、provider clean-home canary 均不可替代。无独立 typed authorization 即 BLOCKED/not-executed；结论仅 PASS/BLOCKED。时序：16 步顺序步骤 12（artifact transport 后）；前置=S05 milestone B-1 result 已回传 |
