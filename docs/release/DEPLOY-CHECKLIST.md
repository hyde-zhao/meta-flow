...
# Meta Flow 0.6.5 Deploy Checklist
...
- [ ] GitHub `v0.6.5` tag/release 指向 frozen source OID，四项远端资产摘要与已验收本地资产一致。
...
| 场景 | 验收 |
|---|---|
| clean-home 安装 | 从 exact wheel 安装，不从 checkout import；`meta-flow version` 为 0.6.5/READY |
| 0.6.3 → 0.6.5 | V1 G2 读取为 effective G3；历史 bytes 不变 |
