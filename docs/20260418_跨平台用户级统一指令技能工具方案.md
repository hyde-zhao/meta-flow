...
> - 2026-04-20 二次评审：废止共享 template，改用平台原生渲染；把运行输出从 meta 仓库 `.output/` 迁移到目标工作区 `.meta-workflow/`；补充运行时优先级与覆盖链。
> - **2026-04-21 三次评审**：
...
>   5. **调整 Phase 顺序**：`scripts/install.py` 平台渲染面修正前置到 Phase 2；
...
7. 仓库 `scripts/install.py`、`README.md`、`platform-validator` 仍保留旧假设；
...
| 编号 | 状态 | 主题 | 简述 |
|------|------|------|------|
| Q-01 | in-progress | 相对路径依赖 | `scripts/`、硬编码仓库路径需改为面向目标项目根目录的路径合同 |
...
| Q-11 | in-progress | Skill 注册表闭环 | 影响 Agent/Skill 关系变更时必须回写 `skills/README.md` |
...
| Q-16 | in-progress | 仓库实现漂移 | `scripts/install.py`（Codex `.yaml`/`.codex/skills`）、`README.md`、`platform-validator` 需同步整改 |
...
| Q-22 | decided | `.meta-workflow/` 噪音控制 | 主选：安装器自动建议把 `.meta-workflow/process/` 与 `checkpoints/` 加入 `.gitignore`，仅 `delivery/` 默认入库 |
...
Canonical source：`agents/`、`skills/`、`rules/`、`scripts/`。
...
统一改为：`{{WORKSPACE_ROOT}}/.meta-workflow/`。
...
1. **注册表**：`skills/README.md` 维护 Skill 注册表；新增 `agents/README.md` 维护 Subagent 注册表；两者都纳入 canonical source。
...
安装器在初始化 `.meta-workflow/` 时默认提示用户把前两者加入 `.gitignore`，用户可关闭此建议。
...
| `meta-dev` | LLD 与实现物走新结构；影响 Skill 关系时同步 `skills/README.md` |
...
**退场条件**：(a) 所有 Skill 的默认写路径都已指向新路径；(b) `scripts/install.py`、`platform-validator`、`README.md` 不再引用 `.output/`；(c) 连续一个迭代周期 doctor 未触发"旧路径依赖"提示。
...
1. `scripts/install.py` 仍把 Codex agent 写为 `.yaml`（应为 `~/.codex/agents/*.toml` / `.codex/agents/*.toml`，遵循 §7.9 写作规范）；
2. `scripts/install.py` 仍写 `.codex/skills/*.md`（应为 `~/.agents/skills/<name>/SKILL.md` 目录形态，Codex 官方 USER scope）；
3. `README.md` 仍保留"Codex 自动转 `.yaml`"旧表述；
4. `platform-validator` 仍围绕 `.codex/skills/` 旧目标面；
5. 根目录 `agents/` / `skills/` / `rules/` 与 meta 内部旧路径未迁到 `{{WORKSPACE_ROOT}}/.meta-workflow/`；
6. 新增：`agents/README.md` Subagent 注册表尚未建立（Q-26 §4.5 要求）。
...
- **背景**：元工作流必须知道"当前被服务的项目根"，以决定 `.meta-workflow/` 写到哪里。二次评审要求显式输入，但这对 CLI / IDE / worktree 场景不友好。
...
### Q-22 `.meta-workflow/` 噪音控制 · decided
...
- **实施位置**：§4.5 全局命名与冲突检测；安装器实现、`skills/README.md` 注册表、新增 `agents/README.md` 注册表。
...
1. 废除默认 `.output/` 根路径；
2. 引入 `{{WORKSPACE_ROOT}}/.meta-workflow/`；
...
> **本 Phase 前 `scripts/install.py` 必须同轮次整改**，否则新路径无法验证。

1. 扫描所有 `.output/...` 引用；
2. 按第 6 节整改 meta Skill；
3. 同步修订 meta Agent 路径合同；
4. 定义 canonical agent 共享字段白名单与 renderer 映射矩阵（Q-14 / Q-18）；
5. **同步修 `scripts/install.py`**：Codex agent → `.toml`（§7.9 规范）、Codex skills → `~/.agents/skills/<name>/SKILL.md`、`README.md` 与 `platform-validator` 表述同步；
6. **新增 `agents/README.md` Subagent 注册表**，与 `skills/README.md` 协同构成 §4.5 冲突检测的数据源。
...
1. 影响 Agent / Skill 关系时同步更新 `skills/README.md`；
2. 固化用户自己执行的边界；
3. 固化权限按需加载与渲染后提示规则；
...
4. meta 输出根改为 `{{WORKSPACE_ROOT}}/.meta-workflow/`，`WORKSPACE_ROOT` 有明确来源合同（Q-04）；
...
3. **元工作流结果写入显式 `WORKSPACE_ROOT` 下 `.meta-workflow/`**；
...
8. **先完成 meta Skill 路径整改、迁移兼容、`scripts/install.py` 同步整改**，再做渲染闭环；
