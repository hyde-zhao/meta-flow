...
description: >-
  ...
  风险、发布与回滚策略，并交由单 Story 或小批次 LLD 确认后再进入实现。触发词包括：LLD、详细设计、实现设计、Story 设计。
...
基于 Story 卡片、已确认的 HLD、架构约束、依赖类型和文件所有权，输出可直接指导编码、评审与验证的 Story 级 LLD，并在 LLD 确认前停止，不进入实现。
...
- LLD `confirmed=false`、`dev_gate` 未满足或文件所有权冲突时不得进入实现
- 不超出当前 Story 范围
...
- [ ] 失败路径覆盖 blocked / 回退 / Spike / LLD 确认 / dev_gate 阻断
