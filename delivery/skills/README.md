...
|---|---|---|
| `state-router` | `host-orchestrator` | 状态机推进与回退，并维护 `workflow_mode`、`orchestrator_session.subagent_auto_dispatch`、`delegated_interaction`、`agent_lifecycle.active_agents`、子 agent 调度证据、`parallel_execution` 队列、`lld_clarification_queue`、`human_gate_decisions.pending_human_decisions`、`human_gate_decisions.decision_collection_coverage`、`cr_tracking`、CR 状态盘点、依赖门控与复用/关闭登记 |
