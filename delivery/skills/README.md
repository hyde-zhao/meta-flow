...
12. 功能 Agent 与高频 Skill 必须遵守 `delivery/rules/AGENT-SKILL-CONTRACT.md`，运行时 source/mirror/path/status/checkpoint 语义以 `delivery/rules/DELIVERY-RUNTIME-CONTRACT.json` 为唯一机器 owner。默认机器状态入口是 `process/state/STATE.current.json`，默认读取集合来自 context / packet 的 `allowed_reads` / `must_read`；`process/STATE.md`、`process/DEVELOPMENT-PLAN.yaml`、完整 CR、全量 Story LLD、完整 TEST-REPORT / REVIEW / diff 属于 `do_not_read_by_default`。
13. `process/...` 是过程仓逻辑引用。任何文件系统 I/O 前必须调用 `meta-flow project resolve-ref --project-root <release-root> --logical-ref <process/...> --format json`；`resolved_path` 只可瞬时使用，退出码 2 必须阻断，不得自行拼 sibling、去前缀、恢复软链接或回退 legacy。

## Agent → Skill 关系

| Agent | 主要阶段 / 场景 | 使用 Skill | 用途 |
|---|---|---|---|
| `host-orchestrator` | `init`、状态推进、变更管理、问题分流、并行调度、检查点控制 | `state-router`、`checkpoint-manager`、`change-impact-analysis`、`issue-routing`、`context-handoff`、`context-manifest-builder`、`review-artifact-protocol` | 推进状态、受理变更、路由问题、装配交接上下文，维护 context pack / workflow health ref、阶段委托交互、LLD clarification question broker、LLD / Dev / QA 并行队列，生成和收敛 CP0-CP8 检查点，维护 CP2/CP3 discussion log/checkpoint、关键决策门控、Decision Brief、`GATE-LEDGER.ndjson`、人工门禁发起消息、CP8 follow-up tracking、`CR-INDEX.json` / `CR-LEDGER.ndjson`、fast-lane、Codex 动态 reasoning profile 路由和自动子 agent 调度证据 |
...
| `meta-dm`（归档） | 历史 Story 规划 | `phase-designer`、`wave-planner`、`dependency-mapper`、`story-manager`、`dag-validator` | 已从交付面移除，仅保留 `delivery/doc/archive/meta-dm.md` 供追溯；现由 `meta-se` 接管 |

## Skill → Canonical Agent 关系

| Skill | Canonical Agent | 说明 |
|---|---|---|
| `state-router` | `host-orchestrator` | 状态机推进与回退，并维护 `workflow_mode`、`subagent_auto_dispatch`、`active_delegation_ref`、`AGENT-DISPATCH-LEDGER.ndjson`、Codex reasoning profile 路由、子 agent 调度证据、并行执行 context ref、`QUESTION-LEDGER.ndjson`、Decision Brief、`GATE-LEDGER.ndjson`、`CR-INDEX.json` / `CR-LEDGER.ndjson`、CR 状态盘点、依赖门控与复用/关闭登记；Codex 工具面可用时，`mode=subagent` handoff 后必须真实调用 `spawn_agent` / `resume_agent` / `send_input` |
...
| `change-impact-analysis` | `host-orchestrator` | 需求/设计变更管理；负责文档处理决策、旧基线映射、CR 执行链路、fast-lane 升级判定、自动终验授权、CP8 follow-up tracking 台账、`CR-INDEX.json`、CR 跟踪一致性检查和变更追溯门禁 |
...
| `lld-designer` | `meta-dev` | Story 设计证据生成；按 `lld_policy` 和 gate profile 输出完整 LLD、Batch LLD Story 锚点、Story 技术说明或 waived 证据，并行 LLD 阶段通过 clarification queue 记录实现灰区，输出后等待全部目标 Story 的设计证据统一确认，不直接进入实现 |
...
| `CP0-CP8 检查结果` | `checkpoint-manager` | `state-router`、`host-orchestrator`、`meta-pm`、`meta-se`、`meta-dev`、`meta-qa`、`meta-doc` | 自动机器结果写 `process/checks/CP*.result.json`，人类摘要写同 stem `*.summary.md`；CP2 / CP3 / CP5 / CP8 人工审查稿写 `process/checkpoints/CP*.md`；CP4 自动结果汇入 CP5；人工门禁发起前必须通过 Decision Brief 与发起消息校验；state-router 以机器结果判定是否可推进 |
| `STATE.current.json` / ledgers / context refs | `state-router` | `checkpoint-manager`、`context-handoff`、`context-manifest-builder` | `process/state/STATE.current.json` 只保存当前 scalar 状态和轻量 refs；CR 状态写入 `CR-LEDGER.ndjson`、`CR-INDEX.json` 和 summaries；人工门禁写入 `process/checkpoints/CP*.md` 与 `GATE-LEDGER.ndjson`；上下文预算写入 read policy、context packs 和 `READ-EXPANSION-LEDGER.ndjson`；workflow health 通过 `workflow_health_ref` 指向当前健康摘要；调度证据写入 handoff dispatch 与 `AGENT-DISPATCH-LEDGER.ndjson`；LLD 实现灰区写入 `QUESTION-LEDGER.ndjson` 或 CP5 context queue ref；`process/STATE.md` 只是 generated human summary / legacy fallback / audit input |
...
| `STORY-*.md` | `story-manager` | `state-router`、`wave-planner`、`lld-designer`、`implementation-execution`、`verification-execution`、`meta-dev`、`meta-qa`、`host-orchestrator` | Story 卡片包含依赖类型、file_ownership、`feature_design_refs`、`lld_policy`、`lld_gate`、`dev_gate`、`implementation_gate` 和 `verification_gate`；`technical-note` / `waived` 的正式设计证据也写在 Story 卡片中，`batch-lld` Story 在 `lld_gate.design_evidence_type` 和 evidence path 指向 Batch LLD 锚点 |
| `STORY-*-LLD.md` / `BATCH-*-LLD.md#story-story-{id}` | `lld-designer` | `implementation-execution`、`verification-execution`、`meta-dev`、`host-orchestrator`、`meta-qa` | 高风险 Story 使用独立 `STORY-*-LLD.md`；`standard-lite` / `allows_batch_lld=true` 下低中风险且共享实现面的 Story 可使用 Batch LLD，但每个 Story 必须有独立锚点；全部目标 Story 的完整 LLD / Batch LLD Story 锚点 / 技术说明 / waived 证据统一确认后，开发与验证均直接消费对应工件 |
...
| `lane-architecture` | `meta-se` | Architecture Gray Areas、边界、依赖、ADR 与计划一致性 | `HLD.md`、`ARCHITECTURE-DECISION.md`、`DEVELOPMENT-PLAN.yaml`、legacy Story 视图、`STORY-*-LLD.md` |
...
2. 第 2 阶段：扩展到 `ARCHITECTURE-DECISION.md` 与 `DEVELOPMENT-PLAN.yaml` / legacy Story 视图。
