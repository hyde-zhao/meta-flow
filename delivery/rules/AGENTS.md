...
- **人工检查点**：所有人工确认统一由 meta-po 发起；Claude Code 可使用结构化选择，Codex 优先使用结构化选择 UI，无法提供时显式降级为 exact 文本确认（`1/approve/通过`、`2/修改: ...`、`3/reject/不通过`）
...
- meta-dev 至少消费：文件影响范围、接口设计、异常处理、测试设计、实施步骤、回滚策略，并在 Story Package 确认后复用同一子 agent 继续实现。
