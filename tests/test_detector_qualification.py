...
def test_repository_r13_incremental_hard_gate_is_currently_pass() -> None:
    ...
    assert report["qualification"] == "source-bounded-incremental-hard-gate-v1"
    assert report["historical_calibration"]["unresolved_file_writer_calls"] == 1053
    assert report["unresolved_unallowlisted_count"] == 0
    assert report["allowlisted_dynamic_writer_call_count"] >= 1
